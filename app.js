...
<body>
  ...
  <script>
    (function(){
      ...
      function renderGallery(variant){var section=make("section","section");section.append(make("h3","","图库"));var gallery=make("div","gallery");var items=(variant.images||[]).filter(function(image){return image.dataUrl;}).map(function(image){return {src:image.dataUrl,title:image.name||"图库图片",description:image.caption||""};});items.forEach(function(item,index){gallery.append(imageFigure(item.src,item.title,item.description,"",items,index));});section.append(gallery.children.length?gallery:make("p","empty","这个分支还没有图库图片。"));main.append(section);}
